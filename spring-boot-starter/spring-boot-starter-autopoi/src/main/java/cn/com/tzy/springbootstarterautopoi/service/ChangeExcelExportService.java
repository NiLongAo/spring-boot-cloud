package cn.com.tzy.springbootstarterautopoi.service;
...
import cn.com.tzy.springbootstarterautopoi.utils.PoiStyleUtils;
...
/**
 * 改造 ExcelExportService
 * 解决相关问题
 * 1.
 */
public class ChangeExcelExportService extends BaseExportService {
        ...
        for (int i = 0, exportFieldTitleSize = excelParams.size(); i < exportFieldTitleSize; i++) {
            ...
            if (entity.getList() != null) {
                ...
                if (StringUtils.isNotBlank(entity.getName()) && sTitel.size() > 1) {
                    ...
                    //重新赋样式
                    PoiStyleUtils.mergedStyle(sheet,titleStyle,new CellRangeAddress(index, index, tempCellIndex, tempCellIndex + getFieldLength(sTitel)));
                }
                ...
                //重新赋样式
                PoiStyleUtils.mergedStyle(sheet,titleStyle,new CellRangeAddress(index, index + rows - 1, cellIndex, cellIndex));
            }
            cellIndex++;
        }
        if (groupCellLength > 1) {
            ...
            //重新赋样式
            PoiStyleUtils.mergedStyle(sheet,titleStyle,new CellRangeAddress(index, index, cellIndex - groupCellLength, cellIndex - 1));
        }
        return cellIndex;
    ...
    public int createTitle2Row(ExportParams entity, Sheet sheet, Workbook workbook, int fieldWidth) {
        ...
        //重新赋样式
        PoiStyleUtils.mergedStyle(sheet,getExcelExportStyler().getHeaderStyle(entity.getHeaderColor()),new CellRangeAddress(0, 0, 0, fieldWidth));
        if (entity.getSecondTitle() != null) {
            row = sheet.createRow(1);
            row.setHeight(entity.getSecondTitleHeight());
            CellStyle headerStyle = getExcelExportStyler().getHeaderStyle(entity.getHeaderColor());
            headerStyle.setAlignment(HorizontalAlignment.RIGHT);
            createStringCell(row, 0, entity.getSecondTitle(), headerStyle, null);
            for (int i = 1; i <= fieldWidth; i++) {
                createStringCell(row, i, "",headerStyle , null);
            }
            PoiMergeCellUtil.addMergedRegion(sheet, 1, 1, 0, fieldWidth);
            //重新赋样式
            PoiStyleUtils.mergedStyle(sheet,headerStyle,new CellRangeAddress(1, 1, 0, fieldWidth));
            return 2;
        }
        return 1;
    }
    ...
    protected void insertDataToSheet(Workbook workbook, ExportParams entity,
        ...
        try {
            ...
            if (entity.isAddIndex()) {
                ExcelExportEntity excelExportEntity = indexExcelEntity(entity);
                excelExportEntity.setOrderNum(-1000);//越界bug
            ...
            setCellWithCope(excelParams, sheet); // TODO: 2022/10/28 修复多级表头宽度问题
    ...
    // TODO: 2022/10/28 修复 多级表头宽度问题  原码只能为二级表头宽度
    private void setCellWithCope(List<ExcelExportEntity> excelParams, Sheet sheet) {
        Map<Integer,Double> map = new HashMap<>();
        getExcelExportWidth(map,excelParams);
        for (Map.Entry<Integer, Double> integerDoubleEntry : map.entrySet()) {
            sheet.setColumnWidth(integerDoubleEntry.getKey(), (int) (256 * integerDoubleEntry.getValue()));
        }
    }

    // TODO: 2022/10/28 遍历树 结构下级 宽度
    private void getExcelExportWidth(Map<Integer,Double> map,List<ExcelExportEntity> excelParams){
        for (ExcelExportEntity excelParam : excelParams) {
            List<ExcelExportEntity> list = excelParam.getList();
            if(list != null){
                getExcelExportWidth(map,list);
            }else {
                map.put(map.size(),excelParam.getWidth());
            }
