    ...
    public <T extends PubSubMessage> void subscribe(PubSubType type, PubSubListener<T> listener, Class<T> clazz) {
        ...
        AbstractMessageListener abstractMessageListener = new AbstractMessageListener(name) {
            ...
            public void onMessage(Message message, byte[] pattern) {
                ...
                if(pubSubMessage instanceof JoinLeaveMessage){
                    SocketIOServer bean = SpringUtil.getBean(SocketIOServer.class);
                    String namespace = ((JoinLeaveMessage) pubSubMessage).getNamespace();
                    if(bean.getNamespace(namespace) == null){
                        return;
                    }
                }else if(pubSubMessage instanceof DispatchMessage){
                    String namespace = ((DispatchMessage) pubSubMessage).getNamespace();
                    SocketIOServer bean = SpringUtil.getBean(SocketIOServer.class);
                    if(bean.getNamespace(namespace) == null){
                        return;
                    }
