...
public class MyTokenServices implements AuthorizationServerTokenServices, ResourceServerTokenServices, ConsumerTokenServices, InitializingBean {
    ...
    public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication) throws AuthenticationException {
        ...
        if (existingAccessToken != null) {
            ...
            Collection<OAuth2AccessToken> auth2AccessTokens = tokenStore.findTokensByClientIdAndUserName(oAuth2Request.getClientId(), name);
            //每个端用户只能有两处登录
            List<OAuth2AccessToken> collect = auth2AccessTokens.stream().sorted((a, b) -> (int) (a.getExpiration().getTime() - b.getExpiration().getTime())).collect(Collectors.toList());
            List<OAuth2AccessToken> oAuth2AccessTokens = collect.subList(0, auth2AccessTokens.size() - 1);
    ...
    public OAuth2AccessToken refreshAccessToken(String refreshTokenValue, TokenRequest tokenRequest)
        ...
        if (!supportRefreshToken) {
            ...
            throw new InvalidGrantException("未开启刷新token权限");
        }

        OAuth2RefreshToken refreshToken = tokenStore.readRefreshToken(refreshTokenValue);
        if (refreshToken == null) {
            log.error("Invalid refresh token: " + refreshTokenValue);
            throw new InvalidGrantException("刷新token已过期");
        }
    ...
    private OAuth2RefreshToken createRefreshToken(OAuth2Authentication authentication) {
        ...
        if (validitySeconds > 0) {
            return new OAuth2ExpiringOAuth2RefreshToken(value, new Date(System.currentTimeMillis()
                    + (validitySeconds * 1000L)));
        }
        return new OAuth2ExpiringRefreshToken(value);
    }
