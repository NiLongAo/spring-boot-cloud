...
public class AckRequestProcessor extends AbstractSipRequestEvent implements SipRequestEvent {
    ...
    public void process(RequestEvent evt) {
        ...
        ParentPlatformVo parentPlatformVo = parentPlatformVoService.getParentPlatformByServerGbId(platformGbId);
        // 取消设置的超时任务
        ...
        SendRtp sendParentPlatformRtp = sendRtpManager.querySendRTPServer(parentPlatformVo.getServerGbId(), null, null, callIdHeader.getCallId());
        if (restResult != null) {
            startSendRtpStreamHand(evt, sendRtpItem,sendParentPlatformRtp, parentPlatformVo, restResult);
        }
    }

    private void startSendRtpStreamHand(RequestEvent evt, SendRtp sendRtpItem, SendRtp sendParentPlatformRtp, ParentPlatformVo parentPlatformVo, MediaRestResult restResult) {
        SendRtpManager sendRtpManager = RedisService.getSendRtpManager();
        if (restResult == null) {
            ...
            if (sendRtpItem.isOnlyAudio()) {
                ...
                // 向上级平台
                try {
                    sipCommanderForPlatform.streamByeCmd(sipServer, parentPlatformVo,sendParentPlatformRtp,null,null);
