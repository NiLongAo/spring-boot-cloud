...
import cn.com.tzy.springbootstarterfreeswitch.enums.*;
import cn.com.tzy.springbootstarterfreeswitch.handler.message.AnswerCallHandler;
...
public class ChannelParkEventHandler implements EslEventHandler {
    ...
    @Resource
    private SendAgentMessage sendAgentMessage;
    @Resource
    ...
    public void handle(String addr, EslEvent event) {
        ...
        AgentInfo agentInfo = RedisService.getAgentInfoManager().get(deviceInfo.getAgentKey());
        if (agentInfo == null) {
            return;
        }
        ...
        if (deviceInfo.getState() != null) {
            switch (deviceInfo.getState()) {
                case "HOLD":
                    ...
                    agentInfo.setAgentState(AgentStateEnum.HOLD);
                    sendAgentMessage.sendMessage(AgentStateEnum.HOLD,agentInfo,ringEntity);
                case "INSERT":
                    ringEntity.setAgentState(AgentStateEnum.INSERT);
                    agentInfo.setAgentState(AgentStateEnum.INSERT);
                    sendAgentMessage.sendMessage(AgentStateEnum.OUT_CALLER_RING,agentInfo,ringEntity);
                case "LISTEN":
                    ringEntity.setAgentState(AgentStateEnum.LISTEN);
                    agentInfo.setAgentState(AgentStateEnum.LISTEN);
                    sendAgentMessage.sendMessage(AgentStateEnum.OUT_CALLER_RING,agentInfo,ringEntity);
                default:
                    break;
            }
        ...
        if (directionEnum.equals(DirectionEnum.OUTBOUND)) {
            ...
            agentInfo.setAgentState(AgentStateEnum.IN_CALL_RING);
            ...
            sendAgentMessage.sendMessage(AgentStateEnum.IN_CALL_RING,agentInfo,ringEntity);
        }
    }
     ...
     * @param ringEntity
     */
    private void outboundCall(CallInfo callInfo, DeviceInfo deviceInfo, AgentInfo agentInfo, CallMessage ringEntity) {
        if (1 == deviceInfo.getDeviceType()) {
            ...
            if (callType == CallTypeEunm.OUTBOUNT_CALL) {
                if (deviceInfo.getCdrType() == 4) {
                    ...
                    sendAgentMessage.sendMessage(AgentStateEnum.TRANSFER_CALL_RING,agentInfo,ringEntity);
                    ...
                    sendAgentMessage.sendMessage(AgentStateEnum.CONSULT_CALL_RING,agentInfo,ringEntity);

                } else if (deviceInfo.getCdrType() == 2) {
                    //先判断坐席是否接通，如果已经接通，则返回当前接通后的状态
                    if (deviceInfo.getAnswerTime() != null) {
                        AgentStateEnum agentState = agentInfo.getAgentState();
                        switch (agentState) {
                            case CONSULT:
                                ...
                                sendAgentMessage.sendMessage(AgentStateEnum.TALKING,agentInfo,ringEntity);
                                break;
            ...
            sendAgentMessage.sendMessage(AgentStateEnum.OUT_CALLED_RING,agentInfo,ringEntity);
        }
        RedisService.getCallInfoManager().put(callInfo);
    }
    ...
    private void sipOutboundCall(String addr,String deviceId,EslEvent event) {
        ...
        String caller = event.getEventHeaders().get("variable_sip_from_user");//主叫号码
        String called = event.getEventHeaders().get("Caller-Destination-Number");//被叫号码
        AgentInfo agent = agentService.getAgentBySip(caller);
        String callId = String.valueOf(FreeswitchUtils.snowflake.nextId());
        ...
        AgentInfo agentInfo = RedisService.getAgentInfoManager().get(agent.getAgentKey());
        if (agentInfo == null) {
