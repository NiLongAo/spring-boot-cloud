...
import cn.com.tzy.springbootstarterfreeswitch.handler.message.RecordCallHandler;
...
public class ChannelBridgeEventHandler implements EslEventHandler {
    ...
    private void sendAgentState(CallInfo callInfo, String agentKey, AgentStateEnum agentState) {
        ...
        AgentInfo agentInfo = RedisService.getAgentInfoManager().get(agentKey);
        if (agentInfo == null) {
            return;
        }
        ...
        sendAgentMessage.sendMessage(AgentStateEnum.TALKING,agentInfo,callMessage);
    }
}
