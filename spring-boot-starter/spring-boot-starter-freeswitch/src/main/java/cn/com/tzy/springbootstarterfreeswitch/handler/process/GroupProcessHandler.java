...
import cn.com.tzy.springbootstarterfreeswitch.enums.CauseEnums;
...
import cn.com.tzy.springbootstarterfreeswitch.strategy.AgentStrategy;
...
public class GroupProcessHandler {
    ...
    private boolean desiganteAgent(CallInfo callInfo, GroupInfo groupInfo, String deviceId) {
        ...
        AgentInfo agentInfo = RedisService.getAgentInfoManager().get(callInfo.getProcessData().get(Constant.DESIGNATE_AGENT).toString());
        if (agentInfo != null && agentInfo.getAgentState() == AgentStateEnum.READY) {
            log.info("callId:{} get desiganteAgent:{} on group:{}", callInfo.getCallId(), agentInfo.getAgentKey(), callInfo.getGroupId());
            //呼叫坐席
    ...
    private boolean memonryAgent(CallInfo callInfo, GroupInfo groupInfo, String deviceId) {
        ...
        GroupMemoryInfo groupMemory = groupMemoryInfoService.find(groupInfo.getId(), callInfo.getCaller());
        if (groupMemory == null) {
            switch (groupMemoryConfig.getFailStrategy()) {
                case 1:
                    return false;
        ...
        AgentInfo agentInfo = RedisService.getAgentInfoManager().get(groupMemory.getAgentKey());
        if (agentInfo != null && agentInfo.getAgentState() == AgentStateEnum.READY) {
            transferAgentProcessHandler.hanlder(callInfo, agentInfo, deviceId);
            agentNotReady(agentInfo);
            return true;
        ...
        for (GroupInfo groupInfo : groupInfos) {
            ...
            while (iterator.hasNext()) {
                ...
                dynamicTask.startDelay("handler_call_queue:"+callQueue.getCallId(),0,()->{
                    ...
                    transferAgentProcessHandler.hanlder(callInfo, agentInfo, callQueue.getDeviceId());
                    agentNotReady(agentInfo);
                });
                //记录最后服务时间
     ...
     * @return
     */
    private AgentInfo getAgentQueue(String groupId) {
        ...
        AgentInfo agentInfo = RedisService.getAgentInfoManager().get(agentQueue.getAgentKey());
        if (agentInfo == null || agentInfo.getAgentState() !=AgentStateEnum.READY) {
            return null;
        }
        return agentInfo;
    ...
    public void agentNotReady(AgentInfo agentInfo) {
        agentInfo.getGroupIds().forEach(groupId -> {
            ...
            log.info("agent:{} not ready for group:{}", agentInfo.getAgentKey(), groupId);
            agentQueues.remove(new AgentQueue(1L, agentInfo.getAgentKey()));
            agentInfoMap.put(groupId, agentQueues);
            if (agentInfo.getAgentState() == AgentStateEnum.TALKING) {
            ...
            if (agentInfo.getAgentState() == AgentStateEnum.AFTER) {
                List<String> callAgentList = callAgents.get(groupId);
                if (!CollectionUtils.isEmpty(callAgentList)) {
    ...
    public void agentFree(AgentInfo agentInfo) {
        ...
        agentInfo.getGroupIds().forEach(groupId -> {
            ...
            log.info("agent:{} ready for group:{}", agentInfo.getAgentKey(), groupId);
            //根据空闲策略
