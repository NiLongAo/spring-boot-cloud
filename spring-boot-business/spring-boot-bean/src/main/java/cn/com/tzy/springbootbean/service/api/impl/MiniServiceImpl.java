...
public class MiniServiceImpl extends ServiceImpl<MiniMapper, Mini> implements MiniService {
    ...
    public RestResult<?> saveMiniUser(MiniUserParam param) {
        ...
        Integer selectCount = miniUserMapper.selectCount(new LambdaQueryWrapper<MiniUser>()
                .eq(MiniUser::getUserId, param.userId).or(o -> o.eq(MiniUser::getMiniId, mini.getId())));
        if (selectCount > 0) {
    ...
    public RestResult<?> unbindMiniWeb(MiniUserParam param) {
        ...
        int delete = miniUserMapper.delete(new LambdaQueryWrapper<MiniUser>()
                .and(o -> o.eq(MiniUser::getUserId, param.userId).or(r -> r.eq(MiniUser::getMiniId, param.miniId))));
        if (delete <= 0) {
