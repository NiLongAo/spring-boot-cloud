...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements MenuService {
    ...
    public PageResult page(MenuParam param) {
        ...
        // 查询操作结束...
        List<TreeNode<Menu>> treeNode = TreeUtil.getTree(menuList, Menu::getParentId, Menu::getId,
                Arrays.asList(null, ""));
        // 树转vue路由
    ...
    public RestResult<?> menuPrivilegeTree() {
        ...
        Map<String, List<Menu>> collect = list.stream().filter(obj -> StringUtils.isNotEmpty(obj.getParentId()))
    ...
    public RestResult<?> save(VueRoutes param) {
        ...
        if (param.getPid() != null) {
            ...
            Integer parentPrivilegeCount = baseMapper.selectCount(Wrappers.<Menu>lambdaQuery()
                    .eq(Menu::getType, Menu.MenuType.BUTTON.getType()).eq(Menu::getParentId, parentMenu.getId()));
    ...
    public RestResult<?> findUserTreeMenu(Long userId) {
        ...
        // 当前用户所有权限菜单
        // 所有菜单
        List<Menu> allMenu = baseMapper
                .selectList(new LambdaQueryWrapper<Menu>().eq(Menu::getStatus, ConstEnum.Flag.YES.getValue()));
        ...
        List<TreeNode<Menu>> treeNode = TreeUtil.getTree(new ArrayList<>(userMenu), Menu::getParentId, Menu::getId,
                Arrays.asList(null, ""));
        // 树转vue路由
        List<VueRoutes> routes = findRoutes(treeNode);
    ...
    public RestResult<?> tenantMenuPrivilegeTree(Long tenantId) {
        ...
        Map<String, Menu> mapMap = menuList.stream()
                .collect(Collectors.toMap(Menu::getId, Function.identity(), throwingMerger(), LinkedHashMap::new));
        ...
        // 特殊处理，如果权限不为空则，将权限化作Id，前端进行分配
        Map<String, List<Menu>> collect = newMenuList.stream().filter(obj -> StringUtils.isNotEmpty(obj.getParentId()))
                .collect(Collectors.groupingBy(Menu::getParentId));
        List<Menu> newMenus = newMenuList.stream().peek(obj -> {
            if (StringUtils.isNotEmpty(obj.getAuthCode())) {
                if (CollUtil.isNotEmpty(collect.get(obj.getId()))) {
                    for (Menu menu : collect.get(obj.getId())) {
                        menu.setParentId(obj.getAuthCode());
        ...
        // 转换树结构
        List<TreeNode<Menu>> treeNode = TreeUtil.getTree(newMenus, Menu::getParentId, Menu::getId, null);
        List<Map> maps = AppUtils.transformationTree("children", treeNode);
        return RestResult.result(RespCode.CODE_0.getValue(), null, maps);
    }
