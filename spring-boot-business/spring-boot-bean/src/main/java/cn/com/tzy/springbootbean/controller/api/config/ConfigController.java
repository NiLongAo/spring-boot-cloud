...
import cn.com.tzy.springbootbean.config.init.ConfigInit;
...
public class ConfigController extends ApiController {
    ...
    public RestResult<?> detail(@RequestParam("k") String k){
        Config config = configService.getOne(new QueryWrapper<Config>().eq("k",k));
        return  RestResult.result(RespCode.CODE_0.getValue(),null,config);
    }
