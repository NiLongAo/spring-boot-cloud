...
public class RoleCandidateParserImpl implements CandidateParser {
    ...
    public <T extends CandidateModel> List<String> parse(WorkflowContext workflowContext, T candidateModel) {
        List<Long> roleIds = candidateModel.getValues();
        RestResult<?> restResult = userServiceFeign.findRoleIdList(roleIds);
        if(restResult.getCode()!= RespCode.CODE_0.getValue()) {
            throw new RespException(restResult.getMessage());
        }
        List<User> userList = null;
        userList = AppUtils.convertValue2(restResult.getData(),new TypeReference<List<User>>(){});
